...
1. On session start, run: `crosslink session last-handoff` — read the previous
   session's handoff notes BEFORE proceeding. Any non-durable session
   information (state, decisions, gotchas) must be captured in the handoff
   notes, not in permanent docs.
2. Lock warnings are **KNOWN NON-BLOCKING NOISE**: if `crosslink session work`
   reports a stale lock, do **NOT** investigate it — proceed with the task.
   The lock system fix is tracked separately (#166/#173).
