...
import { Cause, Duration, Effect, Logger, Schedule, Schema } from "effect"
...
describe("session.retry.policy (bounded budget)", () => {
  function boundedAPIError(code: string): SessionV1.APIError {
    return Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
  ...
  function makePolicy(status: SessionStatus.Service) {
    return Schedule.toStepWithMetadata(
      SessionRetry.policy({
        provider: "test",
        parse: Schema.decodeUnknownSync(SessionV1.APIError.Schema),
        set: (info) =>
          status.set(SessionID.make("session-retry-budget"), {
            type: "retry",
            attempt: info.attempt,
            message: info.message,
            next: info.next,
  ...
  it.instance("bounded timeout/stream class stops after RETRY_MAX_RETRIES (3 POSTs total)", () =>
    Effect.gen(function* () {
      const status = yield* SessionStatus.Service
      const step = yield* makePolicy(status)
      const error = boundedAPIError("ProviderResponseStreamError")
      const sessionID = SessionID.make("session-retry-budget")

      // attempts 1 and 2 (retry 1 and 2) are allowed
      yield* step(error)
      yield* step(error)

      // attempt 3 exceeds RETRY_MAX_RETRIES=2 -> schedule stops with Cause.Done
      const stopped = yield* Effect.catchCause(step(error), (cause) =>
        Effect.succeed(cause.reasons.some((r) => r._tag === "Fail" && Cause.isDone(r.error))),
      )
      expect(stopped).toBe(true)

      // Only two retries were scheduled: status shows attempt 2, never 3.
      expect(yield* status.get(sessionID)).toMatchObject({
        type: "retry",
        attempt: 2,
      })
  ...
  it.instance("off-by-one regression: delay sequence [2000, 4000] then stop = 3 POSTs", () =>
    Effect.gen(function* () {
      const status = yield* SessionStatus.Service
      const step = yield* makePolicy(status)
      const error = boundedAPIError("ProviderResponseStreamError")

      const first = yield* step(error)
      const second = yield* step(error)
      const stopped = yield* Effect.catchCause(step(error), (cause) =>
        Effect.succeed(cause.reasons.some((r) => r._tag === "Fail" && Cause.isDone(r.error))),
      )

      // policy returns [attempt, Duration]: output=attempt, duration=delay(attempt)
      // = 2000 * 2^(attempt-1): attempt 1 -> 2000ms, attempt 2 -> 4000ms, then stop
      expect(first.output).toBe(1)
      expect(Duration.toMillis(first.duration)).toBe(2000)
      expect(second.output).toBe(2)
      expect(Duration.toMillis(second.duration)).toBe(4000)
      expect(stopped).toBe(true)
    }),
  )

  it.instance("ZlibError is bounded and stops at attempt 3", () =>
    Effect.gen(function* () {
      const status = yield* SessionStatus.Service
      const step = yield* makePolicy(status)
      const error = boundedAPIError("ZlibError")
  ...
  it.instance("unbounded class (429 retry-after) is not capped by RETRY_MAX_RETRIES", () =>
    Effect.gen(function* () {
      const status = yield* SessionStatus.Service
      const step = yield* makePolicy(status)
      // 429 APIError WITHOUT metadata.code -> unbounded class; honors retry-after
      const error = apiError({ "retry-after-ms": "0" })

      // 4 steps all succeed (would fail if the budget applied to this class)
      for (let i = 0; i < 4; i++) {
        yield* step(error)
      }
      expect(yield* status.get(SessionID.make("session-retry-budget"))).toMatchObject({
        type: "retry",
        attempt: 4,
      })
    }),
  ...
  it.instance("emits per-attempt WARN and terminal WARN=meta.attempt (totalPOSTs=3)", () =>
    Effect.gen(function* () {
      const status = yield* SessionStatus.Service
      ...
      yield* Effect.withLogger(
        Effect.gen(function* () {
          yield* step(error)
          yield* step(error)
          yield* Effect.catchCause(step(error), (cause) =>
            Effect.succeed(cause.reasons.some((r) => r._tag === "Fail" && Cause.isDone(r.error))),
          )
        }),
        logger,
      ...
      const warnMessages = logs
        ...
        .map((log) => JSON.stringify(log.message))
        .join("\n")

      // per-attempt WARN for retry 1 and retry 2
      expect(warnMessages).toContain('"retry"')
      expect((warnMessages.match(/"attempt":/g) ?? []).length).toBeGreaterThanOrEqual(3)
      // terminal WARN carries totalPOSTs = meta.attempt = 3 (NOT 4)
      expect(warnMessages).toContain("retry budget exhausted")
      expect(warnMessages).toContain('"totalPOSTs":3')
    }),
  )
})
