    ...
    const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input: {
      ...
      const text = yield* llm
        .stream({
          ...
          // Title generation is a secondary request: a mid-stream body-idle error
          // must NOT trigger AI-SDK retries. The fork's own 3-POST retry budget
          // governs the main turn; an extra retry here was the #235 row-1
          // 4-POST-vs-3 root cause. Pin to 0 (maxRetries: input.retries ?? 0).
          retries: 0,
