#!/usr/bin/env python
"""Lazy stdio MCP proxy for the output-schema-drift investigation.

This is the #196 ``proxy.py`` copied into the drift worktree with exactly
two deltas, both documented here so the report can state them precisely:

1. ``MANIFEST_PATH`` is taken from the ``PROXY_MANIFEST`` env var when set,
   defaulting to ``manifest.json`` next to this file.  This lets Test A use
   a null-output-schema manifest while Tests B-F use the conforming
   manifest, each in its own clean session, without overwriting files.
2. ``BackendClient.ensure_ready`` passes through every ``BACKEND_*`` env
   var from the proxy's environment into the backend subprocess.  The #196
   proxy only passed ``BACKEND_INSTANCE_ID`` / ``BACKEND_PID_FILE``; the
   drift backend additionally needs ``BACKEND_SCHEMA_MODE`` and
   ``BACKEND_RESPONSE_MODE`` to change its declared/returned output shape
   between manifest-capture time and call time.

Behaviour is otherwise identical to #196: the proxy is an MCP **server**
to ToolRegistry (serving ``tools/list`` from the static cached manifest,
never querying the backend live) and an MCP **client** to the backend
(spawned lazily on the first ``tools/call``, one reused session, respawn
+ single retry if a forwarded call fails).
...
class BackendClient:
    ...
    async def ensure_ready(self) -> None:
        ...
        # Delta 2 vs #196: pass through ALL BACKEND_* env vars so the drift
