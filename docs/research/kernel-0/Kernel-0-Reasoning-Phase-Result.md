...
| Compatible concurrency and required exclusivity | The boundary evaluates the configured conflict invariant over affected changes. | `p1` changes `d1` while `p2` changes `d2`; both orders are valid. Two grants or effects declared incompatible for one scope cannot coexist in a reachable authoritative view. |
