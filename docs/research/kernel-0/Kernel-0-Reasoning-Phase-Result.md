...
This record reports the open derivation, Sol reconciliation, Work Unit adequacy and verification tests, irreducibility review, two semantic falsification/reduction passes, and a bounded realization comparison directed under Crosslink issue #566. The first Astra 6 Medium pass received only the [Evidence Packet](./Kernel-0-Evidence-Packet.md) and the task stated for that pass. Each Astra 6 High pass received a compact current candidate and an open challenge. No pass selected a production realization. This record is a provisional research result, not a change to the Evidence Packet's established requirements.
...
## Bounded realization result and next boundary

The [realization comparison](./Kernel-0-Realization-Comparison.md) evaluates conventional in-memory and transactional software, source verification, verified compilation, exact-code/ISA proofs, and processorless logic against the same semantic obligations. The comparison does not infer assurance from a language, compiler, database, or circuit alone. Each route must account for the authority-evidence ingress, coherent current-view commitment, accepted-content continuity, external-effect closure, and its actual runtime, storage, or hardware assumptions. Verified compiler and RTL routes remain live later assurance alternatives; neither removes persistence and external-interface obligations by itself.

The first architecture challenge exposed a simpler comparator than the initial transaction-store sketch: because the minimum guarantee covers executor loss but not independent authority-service restart, one still-live service can retain the entire authoritative view and accepted content in memory, admit requests from replaceable executors, and publish guarded whole successor states serially. This is the least elaborate first **falsification vehicle**, not a choice of production realization or a durability guarantee across service failure. Any implementation would still need trusted service isolation, complete authority ingress, and no path for executors to mutate authoritative state directly. A process-local lock among independent state holders would not suffice. Serialization of unrelated proposals is permitted by the model even though it is not required.

The challenge also sharpened two existing obligations. First, an acknowledged revocation completed before an affected request begins must precede that request in the concrete commitment order; generic serializability alone does not establish this real-time precedence across caches or replicas. Second, an immutable reference is insufficient for continuity if its only candidate bytes disappear with the executor. Acceptance must retain any content required for continuation or rely on a declared trusted holder whose integrity and availability survive executor loss. These are realization-conformance findings; they do not add a Kernel-0 primitive.

**Next justified work:** construct a finite formal model of the abstract semantics and two-position Work Unit instance before implementation. It must represent non-vacuous successful histories, authority replacement and replay, current-view conflicts including the three-request cycle, executor death around commitment, revocation precedence, and accepted-content loss/bypass. If it survives, a tiny single-service guarded-acceptance experiment can test the trace-to-model mapping and implementation assumptions. That experiment would keep the complete authoritative state and required accepted content in the service, terminate and replace executors, and record definite commit versus caller-unknown outcomes. It would not claim service-restart safety or arbitrary external-action control. No model or implementation was built in this phase.
...
**WHAT:** The Evidence Packet, open Astra derivation, focused frontier reasoning on effect/failure/continuity boundaries, two fresh Astra semantic falsification passes, one Astra architecture challenge, Sol reconciliation, a finite Work Unit adequacy attempt, verification specification, destructive reduction, and targeted primary-source realization comparison.
...
**WHAT-NOT-TESTED:** The finite abstract instance was specified but not machine-checked. No realization correspondence, concrete authority protocol, storage or external-effect mediation, broader failure model, or liveness proof was tested. The packet's underlying source provenance was not re-audited in this bounded session.
