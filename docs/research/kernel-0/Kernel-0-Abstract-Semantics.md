...
For each authority-changing commitment and each affected proposed commitment, the accepted history supplies a coherent relative order. The affected request is judged against authority current at its commitment point in that order. A completed authority change precedes a later initiated affected request. Overlapping operations may resolve either way if the resulting history is coherent. A truthful read made before invalidation cannot justify a commitment ordered afterward. Unrelated commitments need no universal total order, clock, or stored sequence number.
...
| Relative order of affected authority changes and commitments | Otherwise a stale observation can authorize a later invalid commitment. |
