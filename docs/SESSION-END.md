...
5. Model evidence: if models were used this session, record their performance (strengths, weaknesses, failure modes, token cost) into the Model Capability Registry (knowledge page: model-routing-matrix / docs/research/registry/Model-Routing-Matrix.md + docs/research/registry/model-feedback-*.md). This is how the registry stays current.
