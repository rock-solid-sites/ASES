...
The orchestrator has NO filesystem write (edit denied; orchestrator-guard blocks write tools for non-Builder). Per issue #172, provisional option A: the orchestrator writes markdown through CROSSLINK surfaces (crosslink knowledge add/edit, crosslink issue comment, crosslink session end --notes). If this proves insufficient (e.g. dated repo-file notes needed), revisit option B ('crosslink note' command) — part of the project's continual learning. See #172.
...
| Known gotchas | Pointers to knowledge pages/issues documenting them (e.g. #125 hydration, lock staleness, agent drift, flags lie) | never re-document here |
...
2. Never embed a retrospective — point to where it lives (e.g. #168 -> docs/research/combined-verify-server-retrospective.md).
...
The series of issues and decisions that led to this convention:
- #162 (2026-08-03) — Tooling/methodology boundary: tooling work stays tooling, but tooling observations are research evidence.
- #168 (2026-08-04) — Verify-server retrospective intake: sparked 'the orchestrator is often best-placed for retrospectives/handoffs/housekeeping markdown, but has no filesystem write'.
- #172 (2026-08-04) — Mechanism design: option A (existing crosslink surfaces, zero code) provisionally adopted; B (crosslink note command) if insufficient; C (scoped *.md filesystem write) rejected as riskier.
- This document (2026-08-04) — Convention codified on the #172-A assumption.
- Earlier evidence shaping the convention: #127 thin-orchestrator principle (bounded signals); crash-recovery lesson (committed work survives, conversational state may not — docs/research/session-recovery-after-crash.md); handoff examples in session-handoffs/ and handoff-bundle/.
